...
public static class GamePlayerProperty
{
    ...
    private const string PosKey = "StartPos"; // スコアのキーの文字列
    ...
    // （Hashtableに）スタートポジションがあれば取得する
    public static bool TryGetStartPos(this Hashtable hashtable, out int pos)
    {
        if (hashtable[PosKey] is int value)
        {
            pos = value;
            return true;
        }
    ...
    // スタートポジションを取得する
    public static int GetStartPos(this Player player)
    {
        player.CustomProperties.TryGetStartPos(out int pos);
        return pos;
    }

    // スタートポジションを変更する
    public static void SetStartPos(this Player player)
    {
        hashtable[PosKey] = Random.Range(1, 6);
